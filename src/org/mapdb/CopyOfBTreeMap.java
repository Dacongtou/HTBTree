...
public class CopyOfBTreeMap<K,V> extends AbstractMap<K,V>
    ...
    protected static SortedMap<String, Object> preinitCatalog(DB db) {
        ...
        return new CopyOfBTreeMap<String, Object>(db.engine,Engine.CATALOG_RECID,32,false,0,
     ...
     */
    public CopyOfBTreeMap(Engine engine, long rootRecidRef,int maxNodeSize, boolean valsOutsideNodes, long counterRecid,
    ...
    protected static class BTreeIterator{
        ...
        /** unbounded iterator*/
        BTreeIterator(CopyOfBTreeMap m){
            this.m = m;
            hi=null;
            hiInclusive=false;
            pointToStart();
        ...
        BTreeIterator(CopyOfBTreeMap m, Object lo, boolean loInclusive, Object hi, boolean hiInclusive){
            this.m = m;
            if(lo==null){
                pointToStart();
            }else{
    ...
    static class BTreeKeyIterator<K> extends BTreeIterator implements Iterator<K>{
        ...
        BTreeKeyIterator(CopyOfBTreeMap m) {
            super(m);
        }

        BTreeKeyIterator(CopyOfBTreeMap m, Object lo, boolean loInclusive, Object hi, boolean hiInclusive) {
    ...
    static  class BTreeValueIterator<V> extends BTreeIterator implements Iterator<V>{
        ...
        BTreeValueIterator(CopyOfBTreeMap m) {
            super(m);
        }

        BTreeValueIterator(CopyOfBTreeMap m, Object lo, boolean loInclusive, Object hi, boolean hiInclusive) {
            super(m, lo, loInclusive, hi, hiInclusive);
        }
    ...
    static  class BTreeEntryIterator<K,V> extends BTreeIterator implements  Iterator<Entry<K, V>>{
        ...
        BTreeEntryIterator(CopyOfBTreeMap m) {
            super(m);
        }

        BTreeEntryIterator(CopyOfBTreeMap m, Object lo, boolean loInclusive, Object hi, boolean hiInclusive) {
            super(m, lo, loInclusive, hi, hiInclusive);
        }
    ...
    static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
		...
		public Iterator<E> iterator() {
            if (m instanceof CopyOfBTreeMap)
                return ((CopyOfBTreeMap<E,Object>)m).keyIterator();
            else if(m instanceof SubMap)
                return ((CopyOfBTreeMap.SubMap<E,Object>)m).keyIterator();
            else
                return ((CopyOfBTreeMap.DescendingMap<E,Object>)m).keyIterator();
        }
        @Override
    ...
    static final class Values<E> extends AbstractCollection<E> {
		...
		public Iterator<E> iterator() {
            if (m instanceof CopyOfBTreeMap)
                return ((CopyOfBTreeMap<Object,E>)m).valueIterator();
            else
    ...
    static final class EntrySet<K1,V1> extends AbstractSet<Map.Entry<K1,V1>> {
		...
		public Iterator<Map.Entry<K1,V1>> iterator() {
            if (m instanceof CopyOfBTreeMap)
                return ((CopyOfBTreeMap<K1,V1>)m).entryIterator();
            else if(m instanceof  SubMap)
    ...
    static protected  class SubMap<K,V> extends AbstractMap<K,V> implements  ConcurrentNavigableMap<K,V> {
        ...
        protected final CopyOfBTreeMap<K,V> m;
        ...
        public SubMap(CopyOfBTreeMap<K,V> m, K lo, boolean loInclusive, K hi, boolean hiInclusive) {
            this.m = m;
            this.lo = lo;
...
//            }
    ...
    static protected  class DescendingMap<K,V> extends AbstractMap<K,V> implements  ConcurrentNavigableMap<K,V> {
        ...
        public DescendingMap(CopyOfBTreeMap<K,V> m, K lo, boolean loInclusive, K hi, boolean hiInclusive) {
            this.m = m;
            this.lo = lo;
            this.loInclusive = loInclusive;
            this.hi = hi;
...
//            }
    ...
    public NavigableMap<K,V> snapshot(){
        ...
        return new CopyOfBTreeMap<K, V>(snapshot, rootRecidRef, maxNodeSize, valsOutsideNodes,
    ...
    private static void printRecur(CopyOfBTreeMap m, long recid, String s) {
        CopyOfBTreeMap.BNode n = (CopyOfBTreeMap.BNode) m.engine.get(recid, m.nodeSerializer);
    ...
    public static void main(String [] args) {
    	...
    	Engine engine = new StoreDirect(Volume.memoryFactory(false,0L,CC.VOLUME_CHUNK_SHIFT));
        CopyOfBTreeMap treeMap = new CopyOfBTreeMap(engine,CopyOfBTreeMap.createRootRef(engine,BTreeKeySerializer.BASIC,Serializer.BASIC,CopyOfBTreeMap.COMPARABLE_COMPARATOR,0),
                6,false,0, BTreeKeySerializer.BASIC,Serializer.BASIC,
                CopyOfBTreeMap.COMPARABLE_COMPARATOR,0,false);
